...
def process_rdd(time, rdd):
		...
		try:
			...
			hashtag_counts_df = sql_context.sql("select tweetid, no_of_tweets from hashtags order by no_of_tweets desc limit 20")
...
tweet1=tweet.flatMap(lambda x: cleanData(x))
tweet2=tweet1.map(lambda x: (x, 1))

count=tweet2.reduceByKey(lambda x,y:x+y)
count.pprint()

#TO maintain state
totalcount=tweet2.updateStateByKey(aggregate_tweets_count)
totalcount.pprint()
